...
def generate_codebook(data, size_codebook, epsilon=0.00001):
    ...
    while len(codebook) < size_codebook:
        codebook, codebook_abs_weights, codebook_rel_weights, avg_dist = split_codebook(
            data, codebook, epsilon, avg_dist
        )
...
def split_codebook(data, codebook, epsilon, initial_avg_dist):
    ...
    while err > epsilon:
        ...
        closest_c_list = [
            None
        ] * _size_data  # list that contains the nearest codevector for each input data vector
        vecs_near_c = defaultdict(
            list
        )  # list with codevector index -> input data vector mapping
        vec_idxs_near_c = defaultdict(
            list
